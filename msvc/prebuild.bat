...
rem %4 = Configuration (e.g. Release | Debug)
...
REM fix out dir for YASM
set configuration=%4
if not exist %platform%\%configuration%\mpn\. md %platform%\%configuration%\mpn
echo creating outdir for yasm '%platform%\%configuration%\mpn'
cd /D ..\..
...
if exist %sdir%gmp-mparam.h (
	...
	echo use %sdir%gmp-mparam.h
) else (
    call out_copy_rename %bdir%gmp-mparam.h ..\ gmp-mparam.h
    echo use %bdir%gmp-mparam.h
)
